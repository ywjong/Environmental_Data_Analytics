...
theme_set(mytheme) #this command will set mytheme as the default theme throughout the session
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 0, ymax = 50, fill = "green") + #Draw green rectangle at areas where y = 0:50
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") + #Draw yellow rectangle at areas where x = 50:100
  geom_point() + #Add data points of Date at x and Ozone at y axis
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #Added "good" near the top of green rectangle
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + #Added "moderate" near the top of yellow rectangle. hjust moves the text to the right
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #limit x scale to 20170101 to 20181231, create a tag every 2 months, label dates in Month - day format
...
NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, shape = lakename)) +
  geom_point() 
print(NvsP)
