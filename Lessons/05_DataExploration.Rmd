...
# Load package
library(tidyverse)
# After loading packages, you will see that the checkbox in packages is ticked.
# A good practice is to load all the library packages at the top so that they don't get buried in the middle of lines, especially if the lines are long
# Check knit directory if you cannot find the document you knitted
```
...
head(USGS.flow.data) #default is 6 rows, need to specify if you want to look more than or less than 6 rows 
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #select rows (before comma) and columns (after comma)
...
summary(USGS.flow.data$discharge.mean.approval)
