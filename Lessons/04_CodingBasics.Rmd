...
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
#by column is default for matrix unless you use byrow to interfere
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # 
matrix4
#Even if you just specify only nrow or ncol, R can populate matrix on its own. But you cannot not put both nrow or ncol. IF the elements are not enough to fill in the matrix (e.g. 3 elements for 2*2 matrix), the matrix will be formed with a warning, and the elements will loop over again

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]]
#We use double square for lists
...
dataframe1 <- data.frame(d,e,f) #Data frame is up in data in global environment, while vectors are in the global environment values
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # 

#NA is seen as missing value

dataframe1[1:2] # It displays first two columns. In matrix, it will display first two elements instead
dataframe1[,1:2] #Displays first two columns
dataframe1[1:2,] #display first two rows
...
#install.packages("dplyr") # comment out install commands, use only when needed and re-comment, because we only need to install package once
...
library(tidyverse)
#R won's store two versions of same packages, so whatever version last installed will replace any older versions.
```
...
A single ingredient recipe. The function itself will not be stored in global environment, but the outputs will.
```{r}
# Write the recipe, it does not get displayed in global environment
recipe1 <- function(x){
  mix <- x*2
  return(mix)
}
...
Two single ingredient recipes, baked at the same time. The output will be stored in global environment.
```{r}
recipe2 <- function(x){
  mix1 <- x*2
  mix2 <- x/2
...
recipe6 <- function(x){
  ifelse(x<3, x*2, x/2) #Like con function in arcgis
