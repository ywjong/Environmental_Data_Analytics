...
`dplyr` is a package in R that includes functions for data manipulation (i.e., data wrangling or data munging). `dplyr` is included in the tidyverse package, so you should already have it installed on your machine. The d in 'dplyr' refers to dataframe. The functions act as verbs for data wrangling processes. For more information, run this line of code:
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]#the value behind comma specifies which column to use. When it is blank, it means we choose all columns
...
head(NTL.phys.data.surface1) #In subsetted data, each row still retain their row name
dim(NTL.phys.data.surface1)
head(NTL.phys.data.surface2) #When using dplyr filter, the filtered row will be assigned a new row name
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #The factor Paul lake and peter lake has to be in quotation mark so that R can recognize it
...
# what code do you need to use, based on the class of the variable? Use filter with numbers
...
NTL.phys.data.TuesLake9099 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> ANSWER: Because that implies we have to manually inspect the rows in order to make meaningful filtering. Also data might change when we add in new rows or re-load the csv, so using row numbers is not exactly reproducible.
...
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #desc = descending

# Exercise: 
# Arrange NTL.phys.data by temperature, in descending order. 
# Which dates, lakes, and depths have the highest temperatures? June to July, East Long Lake and Hummingbird Lake, 0m to 0.5m depth
...
#NTL.phys.data.temps <- select(NTL.phys.data, lakename & sampledate & temperature_C) #This leads to error because select has more restricted use
?select
```
### Mutate
...
NTL.phys.data.processed <- 
  ...
  #piping can be faster than for loop
```
...
#if row.names = TRUE, the first column will be all the row numbers
```
