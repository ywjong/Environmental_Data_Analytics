...
#Convert the dataframe to a spatially enabled "sf" (spatial feature) data frame
EPAair_PM_avg_sf <- st_as_sf(EPAair_PM_avg,coords = c('SITE_LONGITUDE','SITE_LATITUDE'),crs=4326)

#Do you see a new column name?                
colnames(EPAair_PM_avg_sf) #There is a new geometry column
...
#mapview and plot is kind of interchangeable
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1) 
#projection puts round earth on flat map. Imagine a light projecting a sphere to a wall. The light placement, closeness of light and type of light all lead to different projection style, thus there are many different projection systems/coordinate systems.
...
#All coordinate system will have a datum to project shape of earth
```
...
* What Datum does it use? WGS84


## SPATIAL ANALYSIS, Second attempt
Applying our knowledge of CRS, let's transform our EPA sites from its native geographic coordinate system to a projected one, namely UTM Zone 17N and repeat what we attempted earlier. Then we'll revisit the buffer. UTM is popular in non-continental scale
...
#The X7 has value of zero, and it refers to Durham Armory itself
...
names(huc8_sf)
plot(huc8_sf$geometry)
plot(huc8_sf$SUB)

#View the data as a map
mapview(huc8_sf)
mapview(huc8_sf['SUB'],
        graticule=st_crs(huc8_sf))
plot(huc8_sf['SUB'],
        graticule=st_crs(huc8_sf)) #With no crs specified, the graticule goes in cardinal direction which might not really reflect the orientation of the map
plot(huc8_sf['SUB'],
...
#leaflets deal with WGS84 well, but not UTM
leaflet() %>% addTiles() %>% 
...
#Wrong order, because maps overlap each other
ggplot()  +
...
leaflet() %>% #Create leaflet object
  ...
  addMarkers(data=EPAair_wgs84,popup = ~as.character(`Site.Name`)) %>% addCircleMarkers(data=EPAair_wgs84,popup = ~as.character(`Site.Name`))
...
#my method
huc0302_counties <- huc2_UTM_nc %>%
  filter(SUB== '0302') %>% #the numbers have to be in quotes, because the SUB are not always numeric, but can be a factor
  st_intersection(counties_sf_utm)

#john's method
huc0302 <- huc2_UTM_nc %>%
  filter(SUB== '0302')
huc0302_counties <- st_intersection(counties_sf_utm, huc0302)


#Map the product, showing values by land area
mapview(huc0302_counties['ALAND'], legend=FALSE)
```
...
dim(counties_sf_utm)
dim(demog_df)
dim(counties_sf_utm_join) #number of column = 10+15-1=24
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% #union command combines four rows of centroid into one row
  st_convex_hull() 
...
#useful for interpolations
triCo_centroids_voronoi <- triCo_centroids %>% 
  st_union() %>% 
  st_voronoi()
myMap <- myMap + geom_sf(data = triCo_centroids_voronoi, color = 'purple', fill=NA)
myMap #From the map, you can decide that when a point is located in certain polygon, it should be under which counties
...
theSite_sfp <- st_point(c(userLng,userLat)) #sfp = spatial feature point, has geometry but dont know where is the coordinate reference system
...
#intersects finds what features in counties_sf_utm intersects the polygon in theSite_sfc_transformed
