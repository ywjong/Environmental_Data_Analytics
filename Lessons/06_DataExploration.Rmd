...
format(today, format = "%d/%b/%Y, %B")
format(today, format = "%Y, %B %d")
format(today, format = "%d-%b-%y")
format(today-1, format = "%d-%b-%y")
...
```{r}
# Change date format into yymmdd
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# function that replaces values higher than 181231 into 19, and paste0 command concatenate the 19 or 20 into beginning of current six digit date format
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
?paste0
# Changes early years from 20xx to 19xx
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# Change yyyymmdd character into datetime format
...
> Answer: in many social science surveys, for example, the NAs might have significance (e.g. response rate), so they need to keep NA

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
?complete.cases #Check how many rows are complete (i.e. have no missing value)
summary(complete.cases(USGS.flow.data)) #but need to summarize how many falses and trues, otherwise it will show you whether each entry is true or false
dim(USGS.flow.data)
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean.approval)) +
  geom_bar() #works well for categorical variables
```
...
#Warning message `stat_bin()` using `bins = 30`. Pick better value with `binwidth`. means that R was not sure whether how many bins you want, so it gave an arbitrary number of 30 bins

#specify bin width: every 10 units create a bin
ggplot(USGS.flow.data.complete) +
...
#limit the range of x, thus creating a "zoomed in" version of graph
...
```{r, fig.height = 3, fig.width = 4}
#Create three separate frequency polygons, one for mean gage height, one for min gage height, one for max gage height
ggplot(USGS.flow.data.complete) +
  ...
  scale_x_continuous(limits = c(0, 10)) #cut x range from 0 to 10
#The graph shows that majority of counts occur at around 2.5 gage height mean. Also, we have more counts on minimums and fewer maximums. 
#Useful to have frequency polygons when variables are in similar magnitude (e.g. from 0 to 15 compared to 0 to 3000)

#Plot just mean, but color this one variable by whether it was approved. In this case, color is within the aesthetics argument
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
#Box represents 25th to 75th quartile (i.e. middle 50 percent). Bold line is the median value of the data.
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#gage height mean vs discharge mean. Gage height means river high, so this graph indicates that the higher the river, the more flow is going through
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#The greater the quantity at certain value, the wider the violin is. The draw.quantiles specify where to draw the lines to represent xth quantiles. If you don't specify any quantiles, there will be no lines on violins
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
#the fig.height command above only works when knitting. It does not work in console.
ggplot(USGS.flow.data.complete) +
