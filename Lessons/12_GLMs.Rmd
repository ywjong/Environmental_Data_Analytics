...
#using the regression, we predict that irradiance = 486.8 - 95.89*depth + error
#r2 = correlation^2
...
> ANSWER: Depth has significant impact on level of irradiance (linear regression, 'Adjusted R2 = 0.3,, df = 15549 p < 0.0001)
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater, color=lakename)) +
    geom_point(size=0.1, alpha=0.5) +
    xlab("Depth")+
    ylab("Water Irradiance")+
    scale_y_log10() +
...
#The intercept means the dissolved oxygen level at 0m depth
#total phosphorus = 6+1.5(depth)+0.93(oxygen)
...
TPplot2 <- ggplot(PeterPaul.chem.nutrients, 
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() #if there is NAs, the corr plot will display ?, i.e. it wont work
...
step(TPAIC) #The lower the aic value, the better
...
ANCOVA adds multiple levels of alpha instead of single alpha in ANOVA
...
#The intercept 353 means paul lake at depth 0, while peter lake on its own has 353+135 total nitrogen
#Total nitrogen = 353 + 135*Peter Lake - 97(depth)
#in this case, the categorical factor peter and paul explains most difference instead of depth

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
#The intercept represents Paul Lake at zero meters, peter lake is 185 higher than paul lake, for paul lake, the depth increases by 19.87, but for peter, the lake depth will decrease total n by 48
