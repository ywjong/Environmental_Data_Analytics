...
In many case, we are not expecting linear relationship. So instead of linear model, we will use nonparametric test.
...
We will run a Mann-Kendall and a Seasonal Mann-Kendall test today, but there are additional variants of these tests within the package including a correlated Seasonal Mann-Kendall test, a multivariate Mann-Kendall test, a partial Mann-Kendall test, a partial correlation trend test, and a Cox and Stuart trend test. Look into the documentation for these tests to determine which one is appropriate for your purposes. Be careful when using nonparametric time trend test especially for the data assumptions.

### Mann-Kendall Test
A Mann-Kendall test will analyze whether there is a monotonic trend in the response variable over time. Let's use the Mann-Kendall test to investigate whether there is a trend in total phosphorus concentrations in Peter Lake over time. Mann-Kendall does not account for seasonality
...
PeterPaul.nutrients.surface <- 
  ...
  filter(depth == 0) %>%
  filter(!is.na(tp_ug)) #filter out NAs in total phosphorus column
...
#It is not looking at date information, it is just going sequentially
#low p-value means that we reject the null hypothesis of "there is no trend over time"
#z-score shows the sign and magnitude of trend. Positive z score shows positive trend. The larger the absolute value of z score, the more we are confident about a trend
...
#Small p-value means the model probably found a change point
#Choose the largest change point out of all possible change points
...
#The change point is 79 starting from 35th data, i.e. 35+79 = 114th data point is the change point
...
# Running mk test at three sections of data instead of whole data leads to higher p-value because of smaller sample size and also less variability within the segments
#However, it is not recommended to over-parse the information into too many sections because when we parse too much, there is always a chance to find trend somewhere....
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  geom_vline(xintercept = as.Date("1998-01-01"), color="black", linetype="dotted") + #When x-axis is date format, we need to tell ggplot it is a date by putting as.Date
  geom_vline(xintercept = as.Date("1993-05-22"), color="black", linetype="dotted") + #Put any date between 34 and 35 entries, i.e. between May 19 to May 26
  scale_color_manual(values = c("#7fcdbb", "#253494"))
?geom_line

# Mann-Kendall test assumes that all data are independent of each other, therefore our dataset actually dont meet the assumption.
```
...
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, 
                                start = c(2009, 1) ,frequency = 12 ) #MONTHLY
...
#Is there a significant trend in this season? i.e. in each month?
```
